...
- **lilToon texture baking** - During the NDMF build, bakes lilToon's texture adjustments into the textures themselves so the adjusted texture ships instead of the adjustment inputs plus per-pixel shader work: main-color corrections (hue/saturation/gamma, gradation map, adjust mask) and active 2nd/3rd layers into the main texture, the alpha mask into the main texture's alpha channel, and outline tone correction into the outline texture
  ...
  - Layer baking is conservative about visual equivalence: layers are baked only while `_Color` is white and not animated (at runtime the tint applies before layer compositing), the 3rd layer is never baked under a live 2nd layer (compositing order), and decal flipbooks / UV scroll-rotate layers are excluded rather than frozen at an arbitrary build-time frame
  - Honors the pipeline's own filters: textures that are excluded, frozen or size/type-filtered are never baked (the uncompressed bake output would not be recompressed), and a bake never consumes a frozen input texture
