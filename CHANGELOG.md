...
- Added NDMF-based editor localization for English, Simplified Chinese, Traditional Chinese, Japanese, and Korean, with shared language switching in inspectors and Preferences.
...
- **lilToon texture baking** - During the NDMF build, bakes lilToon's texture adjustments into the textures themselves so the adjusted texture ships instead of the adjustment inputs plus per-pixel shader work: main-color corrections (hue/saturation/value/gamma, gradation map, adjust mask) and active 2nd/3rd layers into the main texture, the alpha mask into the main texture's alpha channel, and outline tone correction into the outline texture
  - Uses lilToon's own baker shader (`Hidden/ltsother_baker`) through a standard `Graphics.Blit`, with no lilToon C# API calls; when lilToon is absent the feature passes through silently, and versions newer than the tested range disable baking with a warning instead of guessing
  - Animation-aware: a bake whose input property is driven by animation is skipped so the animation keeps working, and colors (`_Color`, `_OutlineColor`) are never baked so color-change animations stay intact
  - Layer baking is conservative about visual equivalence: layers are baked only while `_Color` is white and not animated (at runtime the tint applies before layer compositing) and while the main UV stays still, the 3rd layer is never baked under a live 2nd layer (compositing order), and layers with decal flipbooks, UV scroll-rotate, non-default UV modes, animated tiling/offset, or runtime features the baker shader cannot reproduce (dissolve, reduced lighting, distance fade, per-layer culling, AudioLink, non-default alpha modes) are excluded rather than mis-baked at an arbitrary build-time state
  - Honors the pipeline's own filters and pins: textures that are excluded, frozen or size/type-filtered are never used as bake targets (the uncompressed bake output would not be recompressed), and a bake never repaints or consumes a protected texture — one pinned by frozen settings or referenced by an animation curve; non-Texture2D inputs such as CustomRenderTextures are never consumed either
  - Consumed inputs (gradation map, layer textures, alpha mask) are cleared on the cloned material and drop out of the upload, independent of the unused-slot removal toggle; baked intermediates are destroyed after the build
...
- **Analysis backends return raw complexity scores** - `ITextureAnalysisBackend.AnalyzeBatch` now returns `Dictionary<Texture2D, float>` (0–1 scores) instead of fully built `TextureAnalysisResult`; score→divisor→resolution conversion moved to the service layer via `AnalysisResultHelper`
  ...
  - Behavior-preserving: final `TextureAnalysisResult` values are unchanged
...
- **Preset compression ordering** - Aggressive/Maximum presets switched from Fast to Combined strategy
  ...
  - All built-in presets now use Combined strategy with calibrated thresholds for monotonic compression progression
