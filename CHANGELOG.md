...
- **lilToon texture baking** - During the NDMF build, bakes lilToon's texture adjustments into the textures themselves so the adjusted texture ships instead of the adjustment inputs plus per-pixel shader work: main-color corrections (hue/saturation/gamma, gradation map, adjust mask) and active 2nd/3rd layers into the main texture, the alpha mask into the main texture's alpha channel, and outline tone correction into the outline texture
  ...
  - Layer baking is conservative about visual equivalence: layers are baked only while `_Color` is white and not animated (at runtime the tint applies before layer compositing) and while the main UV stays still, the 3rd layer is never baked under a live 2nd layer (compositing order), and layers with decal flipbooks, UV scroll-rotate, non-default UV modes or animated tiling/offset are excluded rather than mis-baked at an arbitrary build-time state
