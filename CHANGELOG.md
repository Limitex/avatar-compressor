...
- **Frozen texture tracking for generated textures** - Fixed frozen settings not applying to textures generated by other plugins (e.g., MeshGradient)
  - Added `ObjectRegistryUtils` to track original texture assets through NDMF's ObjectRegistry
  - Frozen skip and frozen settings now correctly match textures that were replaced by earlier plugins in the build pipeline
