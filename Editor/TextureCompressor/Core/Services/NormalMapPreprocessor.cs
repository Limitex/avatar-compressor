...
namespace dev.limitex.avatar.compressor.editor.texture
{
    ...
    /// Handles channel layout differences between BC5, DXTnm, and uncompressed formats.
    ///
    ...
    public class NormalMapPreprocessor
    {
        ...
        /// </summary>
        /// <remarks>
        /// AG-layout sources store normal X in alpha, so their alpha is not semantic.
        /// RGB-layout sources have explicit signed Z, and their alpha may contain meaningful
        /// data (e.g., cutout masks) that should be preserved when targeting BC7.
        /// RG-layout sources with significant alpha also warrant preservation.
        /// </remarks>
        ...
        {
            bool sourceStoresExplicitSignedZ = sourceLayout == SourceLayout.RGB;
            return targetFormat == TextureFormat.BC7
                && sourceLayout != SourceLayout.AG
        ...
        /// - RG (BC5): X in R, Y in G, Z in B (B is ignored during BC5 compression)
        /// - AG (DXTnm for DXT5/BC7): X in A, Y in G, R/B are constants for better compression precision on Y
        ...
        )
        {
            ...
            switch (layout)
            {
                ...
                case NormalChannelLayout.AG:
                    // DXTnm: X in A, Y in G.
                    // Keep R/B at constants so compression precision focuses on G.
                    pixel.r = 255;
                    pixel.g = encodedY;
                    pixel.b = 255;
                    pixel.a = encodedX;
